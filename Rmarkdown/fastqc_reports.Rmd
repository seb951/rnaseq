...
fastqc_statistics= fastqc_wrapper(fq.dir = file.path(getwd(),'..',params$fq.dir),
                qc.dir = file.path(getwd(),'..',params$qc.dir),
                threads = params$threads,
                fastqc.path =params$fastqc.path,
                metadata.dir = file.path(getwd(),'..',params$metadata),
...
# Summary Plots 
```{r, summary, fig.width = 9, fig.height=9,echo =F}


rin_phred = ggplot(fastqc_statistics[[1]], aes(y=mean_quality, x=RIN,col = type)) + 
    geom_point( show.legend = FALSE)+
    ylab("Mean quality per sample (Phred Score)") +
    xlab("RNA Integrity Number") 

rin_nbreads = ggplot(fastqc_statistics[[1]], aes(y=tot.seq, x=RIN,col = type)) + 
    geom_point()+
    ylab("Total Nb of Sequences (10e6)") +
    xlab("RNA Integrity Number") 

hist_phred  = ggplot(fastqc_statistics[[1]], aes(x=mean_quality, color=type)) +
    geom_histogram(fill="white",show.legend = FALSE) + 
    ylab("Mean quality per sample (Phred Score)")

hist_reads = ggplot(fastqc_statistics[[1]], aes(x=tot.seq, color=type)) +
    geom_histogram(fill="white") + 
    ylab("Total Nb of Sequences (10e6)")

rin_phred + rin_nbreads + hist_phred + hist_reads
