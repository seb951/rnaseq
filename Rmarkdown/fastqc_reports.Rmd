...
```{r, requirements,echo=TRUE,message =FALSE}
library(fastqcr)
library(DT)
library(readxl)
library(dplyr)
...
fastqcr_wrapper = function(fq.dir = 'path',
  ...
  #fix (shorten) names
  if(length(grep('i5.',qc.names[1]))==1) {
    qc.names = sapply(strsplit(qc.names,split ='i5.',fixed = T),"[[", 2)
  }
...
qc_plot_collection(qc_read_collection, "Overrepresented sequences")
